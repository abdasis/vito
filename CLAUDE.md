...
- Forms with `useForm` helper (follow existing patterns). Navigation with `<Link>` or `router.visit()` — never raw `<a>` for internal routes.
- Tailwind v4: Use `@import "tailwindcss"`, `@theme` for config, **`gap-*` over margins** for sibling spacing.
...
- Mock external services. Use `SSH::fake()` for SSH operations and `Http::fake()` for HTTP — never make real connections.
